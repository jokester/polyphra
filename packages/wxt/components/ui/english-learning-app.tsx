...
import { Volume2, Wand2, Loader2 } from "lucide-react"
...
export function Modal() {
  ...
  const handleRephrase = async () => {
    ...
    const mockResponses: Record<string, string> = {
      shakespeare: `Hark! ${userText.toLowerCase().replace(/you/g, "thou").replace(/your/g, "thy")}. Verily, 'tis a most wondrous sentiment that doth stir the very essence of one's soul!`,
      sherlock: `Elementary observation: "${userText}" - a statement that, upon careful analysis, reveals the logical conclusion that the speaker possesses a methodical approach to communication.`,
      yoda: `"${userText}", you say. Hmm. Wise words these are, yes. Much to learn from this, there is.`,
      gatsby: `Old sport, "${userText}" - why, that's simply marvelous! It reminds me of those golden summer evenings when hope danced like fireflies across the bay.`,
      churchill: `"${userText}" - Indeed, this statement embodies the very spirit of determination that shall guide us through our darkest hours toward the dawn of victory!`,
    }
